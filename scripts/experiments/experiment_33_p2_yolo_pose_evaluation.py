"""
Experiment 31 -- YOLOv8-pose evaluation. Same fair 61-image subset,
ground-truth calibration convention (isolates pointer-angle prediction
quality), real classical-detector crop at inference (deployment-realistic).

Run from the repo root with:
    uv run python scripts/experiments/experiment_31_yolo_pose_evaluation.py
"""
...
CHECKPOINT_PATH = "runs/pose/yolo_runs/experiment_32/weights/best.pt"
RESULTS_CSV = "experiment_33_results.csv"
TOLERANCE_PCT = 5.0
...
def main() -> None:
    ...
    results, n_accurate, n_no_detection, pct_errors = [], 0, 0, []
    ...
    for reading in candidates:
        ...
        kpts = pred.keypoints.xy[0].tolist()  # [[x,y], [x,y], [x,y], [x,y]] in crop pixel coords, order: center,tip,min,max
        center_x, center_y = kpts[0]
        tip_x, tip_y = kpts[1]
    ...
    with mlflow.start_run(run_name="exp33_p2_yolo_pose_evaluation"):
    ...
    print(f"No detection: {n_no_detection}/{n}")
    print(f"{'':20} {'YOLO Pose':>12} {'Classical':>12}")
